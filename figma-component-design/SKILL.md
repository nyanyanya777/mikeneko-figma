...
姉妹スキル: `figma-ds-edit`（既存DS編集）/ `figma-design-create`（画面新規作成）/ `figma-use`（プラグインAPI）。本スキルは **コンポーネント本体の構造とproperty軸を新規設計する** ときだけ使う。軽い改修は `figma-ds-edit` 側。
...
8. **デッドスペースは設計失敗のサイン**。primary=FIXED で content より大きい値を取って下に余白が残る、counter方向に空きが出る、はやり直し（**TextField 9pxデッドスペース事件の構造的禁止**）。
...
0. **用途と需要の確定〔委譲不可・自分〕**。どの画面のどのスロットで使うか、既存コンポでは何故ダメか、を文章化。同名/類似コンポの存在を inventory で確認（重複作成防止）。
...
- **マネージャー（委譲不可）**: ゲート（軸数3超え、組合せ数爆発、Variable未バインド、既存影響未確認、`remote: true` ファイルで作業しようとしているのを止める）
- **① 偵察**: 既存DS / 命名規則 / 変数の inventory
- **② 設計**: property軸と構造、sizing 方針、組合せ総数の事前計算
- **③ 実装**: Figma 上でコンポ生成・property bind
- **④ レビュー（敵対的）**: 全組合せスクショと既存使用箇所のスクショ比較
...
## Boolean / Visibility slot trick（最重要パターン）

「ある/ない」スロットの標準手順。

1. 該当ノード（helper text frame、leading icon、trailing icon、action）を**全 variant に同じ階層で配置**する。
2. Component properties に Boolean property を `addComponentProperty('showHelper', 'BOOLEAN', true)` で追加。**戻り値の `propId` は `name#hash` 形式**（例：`showHelper#290:0`）。**この propId を必ず保存**。
3. **追加直後に `componentPropertyDefinitions` を再取得して照合**（重複時の suffix で名前が変わる）。
4. **全 variant の該当ノード**の `componentPropertyReferences = { visible: propId }` で bind。**bind値は propId 文字列**、`visible: true/false` を直接書くと bind でなく固定値になる。
...
- bind 値は `characters` で `propId`（string）を指定。
...
- 戻り値の `propId` は **`name#hash` 形式**（例：`showHelper#290:0`）。
- 名前が重複すると Figma 側で suffix が付く → **追加直後に再取得して照合**。
- 削除する場合は `deleteComponentProperty(propId)`。bind 先は自動で detach。

### componentPropertyReferences（bind 種別の完全表）
bind の key は対象により異なる。**`visible` 以外も必須で覚える**。

| bind key | 対象ノード | 必要な property type | 例 |
|---|---|---|---|
| `visible` | 任意ノード（表示有無） | BOOLEAN | `{ visible: 'showHelper#290:0' }` |
| `characters` | TextNode の文字列 | TEXT | `{ characters: 'value#290:1' }` |
| `mainComponent` | InstanceNode の差替先 | INSTANCE_SWAP | `{ mainComponent: 'icon#290:2' }` |
| Variant property名 | nested ComponentSet の variant 選択 | VARIANT（同名同値） | `{ 'state': 'innerState#290:3' }` |
...
## 新規追加 vs 既存改修 vs 旧置換のフロー分岐
**§0 用途と需要の確定時に必ず明示**。以降の章の適用範囲が変わる。

| 種別 | 既存影響レビュー(§7) | 3段移行 | Publish前スクショ比較 |
|---|---|---|---|
...
property を後から触るとき、または default sizing を変えるときに必須。新規追加時はスキップ可。
...
- primary=FIXED で content より大きい値を入れてデッドスペースを残す（**TextField事件再発**）
...
- `figma-design-create`：画面を新規に組む側
- Figma が真実、spec は古い前提
- 公式コンポ優先、自作で代替しない
- 作業指示には即実行
- 要件にない要素を勝手に足さない
- 生hex 禁止、変数バインド
