...
7. **生値（hex直書き、px直書き）禁止＝色・タイポ・spacing・radius・effect は全て Variable / Style バインド（F-QLT-5 → [mikeneko-figma-rules](../../skills/mikeneko-figma-rules/SKILL.md)§品質系）**。バインド詳細は §変数・スタイル必須ルール。
...
1. **既存DSの確認**。命名規則、使われている Variable / Text Style、Boolean property の命名慣行（`show` vs `has` vs `with`）を inventory。**ファイル内既存に従う**。
   - **参照ファースト＝REF宣言（F-PRC-11 → [mikeneko-figma-rules](../../skills/mikeneko-figma-rules/SKILL.md)§進め方系）**: 軸設計（order 2）の前に `lazyweb_search`（このコンポのUIパターン名＋platform、例 "segmented control desktop"）で実参照を1〜3件取得し《クエリ／参照名／なぜ良いか根拠1〜3行＝構造の言語／`[REF:]`タグ》で固定。単体atomでもUIパターン名検索は有効。参照からの要件外要素流用は提案止まり（承認まで実装禁止・F-SRC-2）。
...
| 参照ファースト（REF宣言）〔F-PRC-11〕 | オーダー1（軸設計前） | `lazyweb_search`で実参照1〜3件、各件にクエリ/参照名/構造語の根拠/`[REF:]`タグ。単体atomでもUIパターン名検索は有効 | [mikeneko-figma-rules](../../skills/mikeneko-figma-rules/SKILL.md)§進め方系 F-PRC-11 |
| 業界ベンチマークreport〔F-QLT-8〕 | オーダー6完成後・オーダー7と並走 | 入力=全組合せ並べスクショ＋代表使用文脈でgenerate→全所見処分表が埋まるまで完了不可。単体atomへの画面文脈ノイズ所見は(b)根拠付き棄却で吸収。不通時はfail-closedでユーザーエスカレのみ免除 | [mikeneko-figma-rules](../../skills/mikeneko-figma-rules/SKILL.md)§品質系 F-QLT-8 |
...
### コンポーネント化の単位基準（2コピー則）〔正本〕

- **トリガー: 同一構造の2回目のコピーが発生した時点でコンポーネント化する。** カウントは (i)別画面への複製 (ii)**同一画面の状態違いフレームを作るためのコピー**（画面Aの状態S0/S1フレーム間の複製もコピー1回と数える） (iii)同一画面内の繰り返し（テーブル行・カード並び等）のすべてを含む。〔出典: ユーザー明言〕
- **単位: 複製が同一な最大範囲**で切る。「部品っぽく見えるか」でなく画面/状態フレーム間で逐語コピーされている範囲を実測して境界にする。レイアウト差分（余白・divider・並び順が画面ごとに違う部分）は境界の**外**に出し、画面側のcompositionで組む。
- **状態はコピーでなくVariant/プロパティ軸で持つ。** 状態違いフレームは殻＋プロパティ差し替えで作れる状態が正（選択状態を別コンポのバリアント借用で擬装しない=F-CMP-5）。
- **アトミックデザインは層の語彙**（atom=DS基底/molecule=操作単位の複合/organism=Table級/template以上はコンポ化せずcomposition）として棚の整理に使う。**いつ・どこで切るかの判断は本則（2コピー則＋状態軸＋OOUI意味単位）で行う**。層の名前自体は判断基準にしない。
- 反例の逃がし弁: 1箇所にしか出ず状態も持たない装飾は切らない（design-createの`raw:one-off-decor`）。
...
- （本節は**コンポ内部**のsizing/デッドスペース機械則の正本（入口F-STR-2の委譲どおり）。**画面レベル**のSpacer/auto-layout検出仕様は各スキルのゲート（design-create §8(h)『auto-layout厳守逆監査』／ds-edit全件レビューのauto-layout整合観点）が自前で持つ。）
