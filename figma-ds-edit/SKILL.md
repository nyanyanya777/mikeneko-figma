...
7. **見た目が部品なら実体も部品（手描き偽装禁止）。** タブ/入力/バッジ/ボタン/チップ等「コンポーネントに見える要素」を FRAME+TEXT+矩形 で手描きしない。「自前構成／中身は自前で組む」という方針は**器（Dialog等）の話であって、中の部品まで手描きしてよい意味ではない**。各部品は着手前に②棚卸しでDS実体を索引し、在れば必ず instance 化する。手描きが許されるのは「全Nページ走査済みで該当コンポーネントが本当に無い」と証明できた時だけ（その時もローカルComp化を検討、エスカレーション）。
8. **対象範囲にない要素を勝手に足さない（アンチブリード／default-deny）。** 編集は依頼された対象に限る。「良かれ」で要件外のフィールド/要素を増やさない。特に**記憶・文脈・別プロジェクトに出てくる要素を混ぜない**（例: 古着サービスの「店主に呼んでほしい名前」を無関係な会員登録に足す類）。出典になるのは“今回の依頼”と“この対象ファイルの実体”だけ。足した方が良いと判断した要素は置かず、チャットに「提案」として別枠で出して承認を取る。

## 標準オーダー（0→7）
- **0. 前提確定** — URL/node-id・対象範囲・「何を満たせば完了か」を一文化。曖昧な分岐だけ先に確認。
- **1. 現物確認（読み取り）** — 対象の `get_screenshot`＋`get_metadata`。結論より先に実物と構造を見る。
- **2. 棚卸し（読み取り）** — 既存コンポーネント/変数/テキストスタイルを全走査で積極列挙（下記 runbook）。
...
- **6. 自分の目で最終確認〔自分のゲート〕** — 編集した全カード/全要素を全件確認。件数突合（編集N＝確認N）。**スクショの見た目一致だけで合格にしない**：部品に見える各要素を `get_metadata` で1件ずつ開き、node type が `INSTANCE`（正しいmainComponentを指す・detach無し）であることを確認する＝**instance実体監査**。見た目が正しくても FRAME/TEXT の手描き偽装は不合格。
- **7. 正直な報告** — 変更点・カバレッジ・残課題を明示。

## チーム編成
- **マネージャー（自分・委譲不可）**: スコープ確定／驚く結論の検証ゲート／可逆性トリアージ／全件カバレッジ最終確認／完了・コミット可否。判断だけ、ノイジー作業はしない。
- **① 偵察（Recon／読み取り・Explore型）**: 対象の現物と構造の地固め。
- **② 棚卸し（Inventory・司書／読み取り）**: 既存資産の積極列挙。検索空振りを不在の証拠にしない係。
- **③ プランナー（設計）**: ①②→編集計画＋分類＋可逆性トリアージ。成果物をマネージャーがゲート。
- **④ 実装（Implementer／書き込み）**: use_figma 実行。逐次/領域分割。
- **⑤ レビュー（複数・敵対的／読み取り・全件被覆）**: 実装者と別。カード/領域単位で割り合計で全件カバー。
- **⑥ シンセサイザ／反省（必要時）**: ファンアウト所見の統合・重複除去。事故時は blameless ポストモーテム（技術/マネジメント観点）。
...
- **逆差分監査**: 編集後の対象に、依頼に無い要素が増えていないかを逆向きに突合。出典の辿れない要素（他案件ブリード・スコープ膨張）が出ていたら削除かエスカレーション（鉄則8）。
...
- **「自前構成」を口実に、DSに実体がある部品（タブ/入力等）を FRAME+TEXT で手描きする**（見た目は通るが instance でない＝DS違反。実例: 検索モーダルのタブをDS Tabsがあるのに手描き→全面差し替えの手戻り）。発見ファースト②の不徹底＋⑥でinstance実体監査を怠ると見逃す。
